  ...
  return (
    <header className="w-full sticky top-0 z-50 backdrop-blur-lg bg-black/50 border-b border-white/10">
      <div className="container mx-auto px-4 py-6 flex justify-between items-center">
        ...
        <Link
          href="/"
          className="text-2xl font-bold text-emerald-400 font-mono tracking-tight flex items-center"
        >
          <span className="mr-1 text-white">&gt;</span>Haroon
        </Link>

        {/* Desktop Nav (hide Home) */}
        <nav className="hidden sm:flex gap-6 text-sm">
          {navLinks
            .filter((link) => link.href !== "/") // Exclude Home
            .map(({ href, label }) => (
              <Link
                key={href}
                href={href}
